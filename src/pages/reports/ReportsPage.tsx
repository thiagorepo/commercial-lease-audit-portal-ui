...
export function ReportsPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    setError(null);
    try {
      const query: { status?: ReportStatus; type?: ReportType } = {};
      if (statusFilter !== 'all') query.status = statusFilter as ReportStatus;
      if (typeFilter !== 'all') query.type = typeFilter as ReportType;
  ...
  const handleDelete = async (id: string) => {
    const ok = await reportService.delete(id);
    if (ok) {
  ...
  return (
    <div>
      <PageHeader
        title="Reports"
        breadcrumbs={[{ label: 'Dashboard', href: '/dashboard/overview' }, { label: 'Reports' }]}
        actions={
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => void fetchReports()} className="mt-3 text-sm text-primary font-medium hover:text-primary/80">Retry</button>
          </div>
        ) : (
            ...
            {reports.length === 0 ? (
              ...
              <>
                        ...
                        <tr key={r.id} className="border-b border-border/30 hover:bg-accent transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={() => void handleDelete(r.id)} className="p-1.5 text-muted-foreground/70 hover:text-error-600 hover:bg-error-50 rounded-md transition-colors" title="Delete">
                ...
                <div className="px-5 border-t border-border/50">
                  <Pagination page={page} pageSize={pageSize} total={reports.length} onPageChange={setPage} />
                </div>
              </>
            )}
          </div>
      ...
      <NewReportModal open={newReportOpen} onClose={() => { setNewReportOpen(false); void fetchReports(); }} />
    </div>
  );
}
